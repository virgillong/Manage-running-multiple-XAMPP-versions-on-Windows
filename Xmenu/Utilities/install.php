...
$XamppDirNew = isset($argv[1]) ? $argv[1] : getcwd();
//$XamppDirNew   = $argv[1] ?? getcwd();
$forcedSetup = isset($argv[2]) 
    ? filter_var($argv[2], FILTER_VALIDATE_BOOLEAN)
    : false;

$XamppDirCurrent = isset($argv[3]) ? $argv[3] : "";
...
//$partwampp = substr(realpath(__FILE__), 0, stFrrpos(dirname(realpath(__FILE__)), '\\'));
$partwampp=$XamppDirNew;
...
// Only debug
 ...
 //echo "nonpartition: " . $nonpartition."\n";
 //echo "partwampp: " . $partwampp."\n";
 //echo "awkpart: " . $awkpart."\n";
 //echo "awkparslash: " . $awkpartslash."\n";
 //echo "XamppDirNew: " . $XamppDirNew."\n";
 //echo "forcedSetup: " . $forcedSetup."\n\n";	 
...
if (file_exists($installsysroot)) {
	...
	if ($zeile == "usbstick = 1") {
	    ...
	    //virgil added to fix usb stick awknewdir
    ...
    if ($XamppDirCurrent && $forcedSetup) {
	$right = $XamppDirCurrent;
    } else {
	$right = preg_replace("/\r\n/i", "", $right);
    }
    ...
    echo "\r\nCurrent Directory value will be replaced with the value of New Directory.\n";
	echo "     New Directory: " . $partwampp."\n";
	if ($forcedSetup) {
	    echo "     Current Directory (Forced): " .  $right . "\n\n";
	} else {
	    echo "     Current Directory: " .  $right . "\n\n";
	}
	
    $x = strtolower($partwampp);
    $y = strtolower($right);
    
    if (strtolower($partwampp) == strtolower($right)) {
	echo "\r\n new path: $x is equal to current path: $y";
...
if ($xamppinstaller == "newpath") {
    set_time_limit(0);
    // Automatically act as if option 1 was selected
    $BS = 1;
    sleep(1);
}
...
if (($xamppinstaller == "newinstall") || ($BS == 1) || ($updatemake == "makenew") || ($updatemake == "doppelt")) {
    if ($BS == "1") {
	echo "Refreshing allpaths in config files ... ";                                                                     #\r\n";
    }
    echo "Replace $substit  with   $awknewdir \r\n";
    echo "Configure XAMPP for ";
...
//echo "\r\n__________________________________ awknewdir: $awknewdir ______________________________\r\n";
    $substit = "\"" . $substit . "\"";
    $trans = array(
	"^" => "\\\\^",
	"." => "\\\\.",
	...
	$awkrealm = $awkexe . " -v DIR=" . $awknewdir .
            " -v CONFIG=" . $awkconfig .
            " -v CONFIGNEW=" . $awkconfigtemp .
            " -v SUBSTIT=" . $substit .
            " -f " . $awk;

	if (file_exists($awk) && file_exists($awkexe) && file_exists($configreal)) {

	    exec($awkrealm, $output, $returnCode);

	    if ($returnCode !== 0) {
		echo "AWK execution failed (code $returnCode)\r\n\r\n";
		return;
	    }
	}
	if (file_exists($configtemp) && file_exists($configreal)) {
	    
	    //echo "\r\n\r\n***$configreal: Modifying URL paths to $awknewdir***.\r\n";

	    if (md5_file($configtemp) !== md5_file($configreal)) {
    ...
    //echo "\r\n__________________________________ awkdoublebackslashdir: $awkdoublebackslashdir _______________________________\r\n";
	...
	$awkrealm = $awkexe . " -v DIR=" . $awkdoublebackslashdir . 
		" -v CONFIG=" . $awkconfig . 
		" -v CONFIGNEW=" . $awkconfigtemp . 
		"  -v SUBSTIT=" . $doublesubstit . 
		" -f " . $awk;
	...
	if (file_exists($configtemp) && file_exists($configreal)) {
	    ...
	    if (md5_file($configtemp) !== md5_file($configreal)) {
		if (!copy($configtemp, $configreal)) {
		    echo "$configreal failed\r\n\r\n";
		} else {
		    unlink($configtemp);
		    //echo "$configreal succeeded\r\n\r\n";
		    $changesMade++;
		}
	     } else {
		unlink($configtemp);
		//echo "\r\n$configreal No changes detected\r\n\r\n";
	    }
	} else {
	    //echo "\r\n\r\n***$configreal: Modifying URL paths to $awknewdir***.\r\n";
	    // echo "\r\n     Files do not exist\r\n\r\n";
    ...
    //echo "\r\n__________________________________ awkslashdirr: $awkslashdir _______________________________\r\n";
	...
	echo "\r\n\r\nThere were $changesMade config files with changes\r\n";

	if ($changesMade === 0) {
	     $exitCode=2; //no matches found
	    if ($forcedSetup) {
		$exitCode=3; // no matches found forced setup
	    }
	    
	   
	    echo "     Install.sys not updated\n";
	} else {
	    if (file_exists($installsysroot)) {
		$datei = fopen($installsysroot, 'r');
		unset($newzeile);
	    ...
	    // rewrite intall.sys with the new xampp path
	    //$newzeile[] = install.sys line items
		...
		for ($z = 0; $z < $i + 1; $z++) {
			...
			echo "\r\ninstall.sys updated .... $includenewdir\r\n";
			$exitCode = 0; // successful update
		    } else {
...
// exitCode = 0 config files and install.sys files updated with new paths
// exitCode = 1 nothing to do 
// exitCode = 2 no matches found, install.sys not updated, may indicate issue with install.sys
// exitCode = 3 no matches found, force set
exit($exitCode);
?>
